const form = document.querySelector('.form');

form.addEventListener('submit', function(e){
    e.preventDefault();
    const inputPeso = e.target.querySelector('#peso');
    const inputAltura = e.target.querySelector('#altura');
    const peso = Number(inputPeso.value);
    const altura = Number(inputAltura.value);
    console.log(peso,altura);

    if(!peso){
        setResultado('Peso inválido', false);
        return;
    }
    
    if(!altura){
        setResultado('Altura inválida', false);
        return;
    }

    const imc = getImc(peso,altura);
    const nivelImc = imcNivel(imc);
    const msg = `Seu imc é ${imc} (${nivelImc})`;

    setResultado(msg, true);
})

function imcNivel(imc){
    const nivelImc = ['Abaixo do peso','Peso normal', 'Sobrepeso', 'Obesidade 1', 'Obesidade 2', 'Obesidade 3'];
    if(imc > 39.9) return nivelImc[5];
    if(imc >= 34.9) return nivelImc[4];
    if(imc >= 29.9) return nivelImc[3];
    if(imc >= 24.9) return nivelImc[2];
    if(imc >= 18.5) return nivelImc[1];
    if(imc < 18.5) return nivelImc[0];
}

function getImc(peso,altura){
    const resultadoImc = peso / (altura ** 2);
    return resultadoImc.toFixed(2); 
}

function createP(){
    const p = document.createElement('p');
    return p;
}

function setResultado(message, isValid){
    const resultadoText = form.querySelector('.resultado');
    resultadoText.innerHTML = '';

    const p = createP();

    if(isValid){
        p.classList.add('valueTrue');
    }else{
        p.classList.add('valueFalse');
    }

    p.innerHTML = message;
    resultadoText.appendChild(p);
}
